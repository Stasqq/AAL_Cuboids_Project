...
int Space::getHeight() const { return levels.size(); }

int Space::getCuboidsNumber() const { return cuboids.size(); }

Cuboid *Space::getCuboidPointer(int i) { return &cuboids[i]; }
...
LevelSegmentCords* Space::addCornerOccupied(int levelNumber,int x, int y, int occupiedSizeX, int occupiedSizeY) {
    ...
    if(x==0 && y==0){
        ...
        segmentCords = new LevelSegmentCords(this->x - occupiedSizeX,this->y-occupiedSizeY,this->x,this->y);
    }
    return segmentCords;
}

LevelSegmentCords* Space::isCornerSectorOccupiedInN(int startLevel,int levelNumber, int x, int y, int occupiedSizeX, int occupiedSizeY) {
    LevelSegmentCords* segmentCords = nullptr;
    if(x==0 && y==0){
        for(int i=0;i<levelNumber;i++){
            if(startLevel+i == this->getHeight()) break;
            if(levels[startLevel+i].isPointFree(x,y) && levels[startLevel+i].isPointFree(occupiedSizeX-1,occupiedSizeY-1)){
        ...
        for(int i=0;i<levelNumber;i++){
            if(startLevel+i == this->getHeight()) break;
            if(levels[startLevel+i].isPointFree(x,y) && levels[startLevel+i].isPointFree(occupiedSizeX-1,y-occupiedSizeY+1)){
                if(levels[startLevel+i].isEdgeFree(0,y-occupiedSizeY+1,occupiedSizeX-1,y-occupiedSizeY+1)
        ...
        for(int i=0;i<levelNumber;i++){
            if(startLevel+i == this->getHeight()) break;
            if(levels[startLevel+i].isPointFree(x,y) && levels[startLevel+i].isPointFree(x-occupiedSizeX+1,occupiedSizeY-1)){
                if(levels[startLevel+i].isEdgeFree(x-occupiedSizeX+1,0,x-occupiedSizeX+1,occupiedSizeY-1)
        ...
        for(int i=0;i<levelNumber;i++){
            if(startLevel+i == this->getHeight()) break;
            if(levels[startLevel+i].isPointFree(x,y) && levels[startLevel+i].isPointFree(x-occupiedSizeX+1,y-occupiedSizeY+1)){
                if(levels[startLevel+i].isEdgeFree(x-occupiedSizeX+1,y-occupiedSizeY+1,x-occupiedSizeX+1,y)
                   && levels[startLevel+i].isEdgeFree(x-occupiedSizeX+1,y-occupiedSizeY+1,x,y-occupiedSizeY+1)){
     ...
     do{
         ...
         if(segmentCords != nullptr){
             ...
             for(int j=0;i+j<this->getHeight() && j<=z;j++){
                 if(!levels[i+j].isSectorFree(segmentCords->getX1(),segmentCords->getY1(),segmentCords->getX2()+1,segmentCords->getY2()+1)){
