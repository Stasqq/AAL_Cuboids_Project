...
void SearchingAlgorithm::handleCuboid(Cuboid *cuboid) {
    ...
    if (segmentCords != nullptr) {
        space -> occupySegmentInN(segmentCords -> getStartLevel(), cuboid -> getZSize(), segmentCords -> getX1(),
                                   segmentCords -> getY1(), segmentCords -> getX2() + 1,
        ...
        space -> occupySegmentInN(space -> getHeight(), cuboid -> getZSize(), 0, 0, cuboid -> getXSize(),
                                   cuboid -> getYSize());
    }
}
...
void SearchingAlgorithm::sortCuboidsByBiggestVolume(std::vector <Cuboid> *cuboids) {
    ...
    auto volume_cmp = [](const Cuboid &a,
                         const Cuboid &b) {
