...
void BruteForceAlgorithm::handleCuboidTempSpace(Space* tempSpace,Cuboid *cuboid) {
    ...
    segmentCords = tempSpace->findPlaceForBruteForce(cuboid->getXSize(),cuboid->getYSize(),cuboid->getZSize());

    if(segmentCords != nullptr){
        tempSpace->occupySegmentInN(segmentCords->getStartLevel(),cuboid->getZSize(),segmentCords->getX1(),segmentCords->getY1(),segmentCords->getX2()+1,segmentCords->getY2()+1);
        ...
        tempSpace->occupySegmentInN(tempSpace->getHeight(),cuboid->getZSize(),0,0,cuboid->getXSize(),cuboid->getYSize());
    }
}
